...
all_data = pd.concat(all_df_merge, ignore_index=True)

# Group by the configuration columns and calculate the average user time
average_user_time = all_data.groupby(list(all_data.columns[:-1]))[all_data.columns[-1]].mean().reset_index()
print(average_user_time)
