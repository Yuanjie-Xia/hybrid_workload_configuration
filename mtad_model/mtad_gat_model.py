...
from mtad_model.modules import (
    ConvLayer,
...
class MTAD_GAT(nn.Module):
    ...
    def forward(self, x):
        ...
        h_cat = torch.cat([x, h_feat, h_temp], dim=1)  # (b, n, 3k)
...
def train_model(model, train_loader, optimizer, criterion):
    ...
    for inputs, targets in train_loader:
        optimizer.zero_grad()
        predictions, recons = model(inputs)

        # Assuming your target is a sequence too, adjust the loss computation accordingly
        loss_forecast = criterion(predictions, targets)
        loss_recon = criterion(recons, inputs)
